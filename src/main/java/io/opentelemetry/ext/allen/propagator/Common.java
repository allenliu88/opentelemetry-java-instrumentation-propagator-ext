...
final class Common {
  ...
  public static final ContextKey<String> REQUEST_ID_CONTEXT_KEY =
      ContextKey.named(REQUEST_ID_HEADER);

  // Propagate Request Headers Key
  public static final String PROPAGATE_REQUEST_HEADERS_PROP_KEY =
      "otel.instrumentation.propagate.http.request.headers";
  ...
  static List<String> filterBlanksAndNulls(String[] values) {
    return (List)
  ...
  static ContextKey<String> buildContextKey(String contextKeyName) {
    if (StringUtils.isNullOrEmpty(contextKeyName)) {
      return null;
    }

    return ContextKey.named(contextKeyName.toLowerCase());
  }

  static void infoWithFlag(Logger logger, String message) {
    logger.info("===================" + message + "===================");
  }
}
